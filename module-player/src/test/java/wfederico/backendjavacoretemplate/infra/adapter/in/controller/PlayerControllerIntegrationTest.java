...
class PlayerControllerIntegrationTest {
    ...
    private final PlayerResponseDTO sample = PlayerResponseDTO.builder()
    ...
    void getAll_returns200WithPagedResults() throws Exception {
        Page<PlayerResponseDTO> page = new PageImpl<>(List.of(sample), PageRequest.of(0, 10), 1);
        when(playerUseCase.getAllPlayersPaged(any())).thenReturn(page);
        mockMvc.perform(get("/api/v1/players").param("page", "0").param("size", "10"))
    ...
    void getPlayer_returns200WhenFound() throws Exception {
        when(playerUseCase.getPlayerById(1L)).thenReturn(sample);
    ...
    void createPlayer_returns201OnValidInput() throws Exception {
        when(playerUseCase.createPlayer(any(PlayerRequestDTO.class))).thenReturn(sample);
        mockMvc.perform(post("/api/v1/players")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"first_name":"Lionel","last_name":"Messi",
                                 "position":"Forward","alter_position":"Midfielder"}
                                """))
    ...
    void updatePlayer_returns200OnValidInput() throws Exception {
        when(playerUseCase.updatePlayer(eq(1L), any(PlayerRequestDTO.class))).thenReturn(sample);
    ...
    void patchPlayer_returns200OnPartialUpdate() throws Exception {
        when(playerUseCase.patchPlayer(eq(1L), any())).thenReturn(sample);
        mockMvc.perform(patch("/api/v1/players/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""{ "first_name": "Leo" }"""))
                .andExpect(status().isOk());
    }
