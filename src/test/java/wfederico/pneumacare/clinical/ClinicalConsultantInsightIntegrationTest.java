...
class ClinicalConsultantInsightIntegrationTest {
    ...
    void seed() {
        ...
        // In dev Hibernate creates the tables without the migrations' column defaults,
        // so seed id and the NOT NULL audit/timestamp columns explicitly.
        jdbcTemplate.update("""
                INSERT INTO medical_reference
                    (id, metric, band, range_descriptor, context, guidance_text, source_ref, priority, created_at)
                VALUES
                    (gen_random_uuid(), 'RSBI', 'UNFAVORABLE', '> 105', 'weaning readiness',
                     'RSBI above 105 predicts weaning failure.', 'IT-RSBI', 70, now())
                """);

        evaluationId = UUID.randomUUID();
        jdbcTemplate.update("""
                INSERT INTO evaluations
                    (id, patient_id, shift_id, physical_ventilator_id, evaluation_time, f, vt, pao2, fio2, pplat, peep,
